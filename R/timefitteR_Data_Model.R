...
#'
#' The timefitter functions to create synthetic data sets expect to find a certain defined model. This set of functions builds that model. You almost certainly want to call [timefitteR_Data_Model(interactive = TRUE)] rather than this constructor.
#'
#' @details
#'
#' The model general form is:
#' - Model:
#'     - vars
#'         - timevariable
#'         - time_params
...
timefitteR_Data_Model <- function(interactive = TRUE,
                                      time,
                                      conditions,
                                      predictors,
                                      base_params,
                                      ...
                                      step) {
if(interactive){
  ##DO ALL THE INTERACTIVE DATA COLLECTION
  if(missing(time)){
    while(TRUE){
      t <- readline(prompt = "Enter the name of the variable to be used as for timepoints:")
      if(length(t)>1){
        time <- t
        break
      }
  ...
  print(time)
  return(new_timefitteR_Data_Model(time = time,
                                   conditions = conditions,
                                   predictors = predictors,
                                   base_params = base_params,
                                   condition_params = condition_params,
