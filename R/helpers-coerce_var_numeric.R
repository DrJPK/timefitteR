...
#'
#' @description
#' This function is used **internally** to coerce a given column of a dataframe into a numeric format suitable for time fitting.
#'
#' @details
...
#' timefitteR_coerce_var_numeric(x)
#'
#' x <- as.character(paste0("Time",sprintf("%02d", sample(1:10, 50, replace = TRUE))))
#' timefitteR_coerce_var_numeric(x, prefix = "Time")
#'

.coerce_var_numeric <- function(x,
                                      prefix = NULL,
                                      suffix = NULL) {
  ...
  if (is.factor(x)) {
    rlang::inform(
      ...
      class = "timefitteR_var_coerced_factor"
    )
    return(as.numeric(x))
  }
  ...
  if (is.character(x)) {
    ...
    if (any(new_na)) {
      rlang::abort(
        ...
        class = "timefitteR_var_not_numeric",
        call  = NULL
      )
    }
    ...
    rlang::inform(
      ...
      class = "timefitteR_var_coerced_character"
    )
